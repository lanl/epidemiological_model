...
#                 sigma = np.sqrt(sigma_squared)
    ...
    def fit_constants(self, disease_name):
        try:
            self.fit_out = minimize(self.fit_objective, self.init_fit_parameters(), kws = {'disease_name': disease_name}, method = 'nelder', tol=0.001,    options={'maxiter': 10000, 'disp': True})
    ...
    def calc_ci(self):
        ...
        for i in range(0, len(self.df_list) - 1):
            ...
            lowess = sm.nonparametric.lowess(self.df_list[i + 1]['nll'], self.df_list[i + 1][self.fit_params[i]], frac = .08)
